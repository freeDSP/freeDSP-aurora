...
uint8_t ir_action(uint32_t value) {
    ...
    if (value == 0xFFFFFFFF) {
        ...
        switch ((value & 0x00007F00) >> 8) {
            case 0x50:
                action = REMOTE_BUTTON_PLUS;
                break;
            case 0x10:
                action = REMOTE_BUTTON_PREVIOUS;
                break;
            case 0x7a:
                action = REMOTE_BUTTON_PLAY;
                break;
            case 0x60:
                action = REMOTE_BUTTON_NEXT;
                break;
            case 0x30:
                action = REMOTE_BUTTON_MINUS;
                break;
            case 0x40:
                action = REMOTE_BUTTON_MENU;
                break;
            case 0x3a:
                action = REMOTE_BUTTON_CENTER;
                break;
            case 0x20:
                action = REMOTE_BUTTON_RELEASED; /* emitted when PLAY or CENTER buttons are released */;
                break;
        }
    }
    return action;
