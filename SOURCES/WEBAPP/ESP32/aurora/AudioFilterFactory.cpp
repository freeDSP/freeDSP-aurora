 ...
 */
void AudioFilterFactory::makeParametricEQ(float a[], float b[], const float gain, const float fc, const float Q,
                                          const float fs) {
 ...
 */
void AudioFilterFactory::makeLowShelv(float a[], float b[], const float gain, const float fc, const float S,
                                      const float fs) {
    float A = pow(10.0, gain / 40.0);
    float w0 = 2 * pi * fc / fs;
 ...
 * \param S Defines the slope.
 */
void AudioFilterFactory::makeHighShelv(float a[], float b[], const float gain, const float fc, const float S,
                                       const float fs) {
    float A = pow(10.0, gain / 40.0);
    float w0 = 2.0 * pi * fc / fs;
    float alpha = sin(w0) / 2.0 * sqrt((A + 1.0 / A) * (1.0 / S - 1.0) + 2.0);
