...
String createChannelNamesPage(void) {
    ...
    ret += String("<center><table><tr><td id='audioinputs' data-numchannels='") + String(numInputs) +
           String("'>Audio Inputs</td>");
    ...
    for (int ii = 0; ii < numInputs; ii++)
        ret += String("<tr><td> Input #") + String(ii + 1) + String("</td>")
               + String("<td><input type='text' id='input") + String(ii) + String("' value='") + channelNames[ii] +
               String("'></td></tr>");
    ret += String("</table>");
    ...
    for (int ii = 0; ii < numOutputs; ii++) {
        ret += String("<tr><td> Output #") + String(ii + 1) + String("</td>")
               + String("<td><input type='text' id='output") + String(ii) + String("' value='") +
               channelNames[numInputs + ii] + String("'></td></tr>");
    ...
    for (int ii = 0; ii < NUMPRESETS; ii++)
        ret += String("<tr><td> Preset #") + String(ii + 1) + String("</td>")
               + String("<td><input type='text' id='preset") + String(ii) + String("' value='") + presetNames[ii] +
               String("'></td></tr>");
    ret += String("</table>");
